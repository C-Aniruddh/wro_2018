...
const byte analogPin = A13;   // Connect AN output of LSA08 to analog pin 0
...
void LSA_Manager() {
  ...
  if (digitalRead(junctionPulse)) {
    junctionManager();
    delay(200);
  } else if (positionVal <= 28)
    Left(32, 18);

  // Line at middle when 29 - 42, move forward
  else if (positionVal <= 42)
    Forward(28, 29);

  // Line at right when 42 - 70, move right
  else if (positionVal <= 77)
    Right(18, 32);
...
// Line at left when 0 - 28, move left
