...
class TileSolver(object):
    ...
    def canPlaceTile(self, mat, tile, row, col):
        ...
        # Confirm that, in each place in the tile where there is a non-zero entry, the matrix has a space open (a
        # zero entry)
        for i in range(tileRows):
            for j in range(tileCols):
    ...
    def hasSolution(self, board, tiles, numTiles, rowStart=0, colStart=0, placement=1):
        ...
        # Note, the i, j loops just move to the next empty tile. Since the tile shapes are complex, this is a decent
        # way of doing it.
        for i in range(rowStart, rows):  # For each row
            ...
            for j in range(innerStart, cols):  # For each column
                if board[i][j] == 0:  # If the cell is empty
                    ...
                    for tileNumber in range(len(tiles)):  # Loop through all the tiles
                        if numTiles[tileNumber] > 0:  # If there are more available tiles of this kind to place
                            ...
                            for rot in range(self.uniqueRotations[tileNumber]):
                                ...
                                if self.canPlaceTile(board, tile, i,
                                                     j + colOffset):  # If it works, see if we can solve using the
                                    # new solution
                    ...
                    return False  # There is no tile, or rotated tile, that can fill the cell, so this path can't be
                    # part of a solution.
...
class Tile:
    ...
    def rotateTile(tile):
        tileRows = len(tile)
        tileCols = len(tile[0])
        rotTile = numpy.zeros((tileCols, tileRows),
                              dtype=numpy.int)  # After the rotation, the new matrix has number of rows and columns switched.
