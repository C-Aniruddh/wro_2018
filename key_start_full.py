...
GPIO.setup(btn, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # Button to GPIO23
...
def serial_feedback():
    while sr_feedback.wait(timeout=0.5):
        ...
        while l_str != "E":
            ...
            if l_str == "E":
                ...
                hole_detection.clear()
                block_pickup.clear()
                temp_position_handler("stack_place", shape=current_block)
                actuate_to_value(0)
                print('\x1b[3;30;42m' + 'Stack placed' + '\x1b[0m')


def key_press():
    while key_press.wait(timeout=0.1):
